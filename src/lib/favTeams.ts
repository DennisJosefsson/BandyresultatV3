...
const favTeamsValidator = z
  .array(z.coerce.number())
  .catch([])
export type T = z.infer<typeof favTeamsValidator>
const storageKey = 'favTeams'

export const getFavTeamsServerFn = createServerFn().handler(
  async () => {
...
export const setFavTeamsServerFn = createServerFn({
  method: 'POST',
})
  .validator(favTeamsValidator)
  .handler(async ({ data }) =>
