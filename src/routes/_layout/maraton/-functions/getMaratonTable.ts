      ...
      try {
        const staticTables = db
          .select({
            ...
            seasons: countDistinct(tables.seasonId).as(
              'seasons',
            ),
        ...
        const mainTable = db
          .select({
            ...
            seasons: countDistinct(teamgames.seasonId).as(
              'seasons',
            ),
        ...
        const result = db.$with('result').as(
          db
            ...
            .select({
              ...
              seasons: sum(unionQuery.seasons)
                .mapWith(Number)
                .as('seasons'),
