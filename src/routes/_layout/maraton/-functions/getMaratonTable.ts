...
import { and, asc, count, countDistinct, desc, eq, sql, sum } from 'drizzle-orm'
...
export const getMaratonTables = createServerFn({
  method: 'GET',
})
    ...
    try {
      const staticTables = db
        .select({
          ...
          totalWins: sum(tables.won).mapWith(Number).as('total_wins') as unknown as SQL<number>,
          totalDraws: sum(tables.draw).mapWith(Number).as('total_draws') as unknown as SQL<number>,
          totalLost: sum(tables.lost).mapWith(Number).as('total_lost') as unknown as SQL<number>,
        })
        .from(tables)
        .leftJoin(series, eq(tables.serieId, series.serieId))
        .where(and(eq(tables.category, 'regular'), eq(series.level, 1.0), eq(tables.women, women)))
      ...
      const unionQuery = db.$with('union_query').as(unionAll(staticTables, mainTable))
      ...
      const result = db.$with('result').as(
        db
          ...
          .select({
            ...
            totalPoints: sum(unionQuery.totalPoints).mapWith(Number).as('total_points'),
