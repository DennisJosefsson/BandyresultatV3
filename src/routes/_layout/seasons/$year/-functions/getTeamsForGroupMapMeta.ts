...
export const getTeamsForGroupMapMeta = createServerFn({
  method: 'GET',
})
  ...
  .handler(
    async ({
      ...
      try {
        ...
        const title = `Bandyresultat - Karta - ${women === true ? 'Damer' : 'Herrar'} ${seasonYear!}`
        const url = `https://bandyresultat.se/seasons/${year}/${group}/map?women=${women}`
        const description = `Karta ${seasonYear} ${women ? 'damer' : 'herrar'}`
        const meta = {
          title,
          url,
          description,
