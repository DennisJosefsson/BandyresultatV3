...
export const getPlayoffTableData = async ({ playoffSeason }: FunctionProps) => {
  ...
  const finalGames = await db
    ...
    .where(and(eq(games.seasonId, playoffSeason.seasonId), eq(games.group, 'final')))
  ...
  const semiTables = playoffTables.filter((tbl) => ['S1', 'S2'].includes(tbl.group))
  const quarterTables = playoffTables.filter((tbl) => ['Q1', 'Q2', 'Q3', 'Q4'].includes(tbl.group))
  const eightTables = playoffTables.filter((tbl) => ['E1', 'E2', 'E3', 'E4'].includes(tbl.group))
...
async function getPlayoffAsSeriesTable(seasonId: number) {
  const playoffCte = db.$with('playoff_cte').as(
    db
      ...
      .from(teamgames)
      .where(and(eq(teamgames.seasonId, seasonId), inArray(teamgames.category, ['playoffseries'])))
      .groupBy(teamgames.group, teamgames.teamId, teamgames.category),
  )
  ...
  const seriesData = await db
    .select()
    .from(series)
    .where(and(eq(series.seasonId, seasonId), eq(series.category, 'playoffseries')))
