...
import { parentchildseries, tables, teamgames, teams, teamseries } from '@/db/schema'
import { db } from '@/db'
...
export const getUnionedTables = async ({ serie, table }: FunctionProps) => {
  ...
  const startTable = db
    .selectDistinctOn([teamgames.teamId], {
      ...
      totalPoints:
        table === 'all' ? teamseries.bonusPoints : sql`0`.mapWith(Number).as('total_points'),
  ...
  const unionQuery = parentSerie
    ? db.$with('union_query').as(unionAll(startTable, parentSerie, mainSerie, mixSerie))
  ...
  const result = await db
    ...
    .groupBy(unionQuery.teamId, teams.teamId, teams.name, teams.shortName, teams.casualName)
    .orderBy(
