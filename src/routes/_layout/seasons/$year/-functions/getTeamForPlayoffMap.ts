...
import { county, municipality, playoffseason, seasons, teamgames, teams } from '@/db/schema'
import { db } from '@/db'
...
export const getTeamsForPlayoffMap = createServerFn({
  ...
  .validator(zodValidator(zd.object({ year: zd.int(), women: zd.boolean() })))
  .handler(async ({ data: { year, women } }): Promise<TeamsForPlayoffMapReturn> => {
    try {
