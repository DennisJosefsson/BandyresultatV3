...
type DevelopmentClickerProps = { dates: Array<string> }

const DevelopmentClicker = ({
  dates,
}: DevelopmentClickerProps) => {
            ...
            {dates.map((date, arrIndex) => {
              return (
                <CarouselItem
                  key={date}
                  data-shortarray={
                    dates.length < 5 ? true : false
                  }
                  className="flex data-[shortarray=true]:basis-full basis-1/3 cursor-pointer flex-row items-center justify-center p-0 text-[8px] md:basis-1/5 md:text-sm [.is-snapped]:font-semibold "
                  onClick={() =>
