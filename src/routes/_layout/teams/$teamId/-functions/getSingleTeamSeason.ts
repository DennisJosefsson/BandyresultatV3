...
import { games, seasons, series, teams, teamseasons, teamseries } from '@/db/schema'
import { db } from '@/db'
import {
  getSeasonGames,
  getSeasons,
...
export const getSingleTeamSeason = createServerFn({
  ...
  .handler(async ({ data: { teamId, seasonId } }): Promise<SingeTeamSeasonReturn> => {
    try {
      const team = await db.query.teams.findFirst({
        where: (teamsSchema, { eq: equal }) => equal(teamsSchema.teamId, teamId),
      })
      ...
      const season = await db.query.seasons.findFirst({
        where: (seasonsSchema, { eq: equal, and: AND }) =>
          AND(equal(seasonsSchema.women, team.women), equal(seasonsSchema.year, seasonYear)),
      })
      ...
      const teamSeason = db
        ...
        .where(and(eq(teamseasons.teamId, team.teamId), eq(seasons.year, seasonYear)))
      ...
      const gamesForTeam = await db
        ...
        .where(
          and(
            or(eq(games.homeTeamId, team.teamId), eq(games.awayTeamId, team.teamId)),
      ...
      const tableSeriesArray = seriesForTeam
        .filter((serie) => ['regular', 'qualification'].includes(serie.category))
