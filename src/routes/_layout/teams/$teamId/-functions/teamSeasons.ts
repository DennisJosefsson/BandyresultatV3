    ...
    async ({ data: teamId }): Promise<TeamSeasonReturn> => {
      try {
        ...
        if (!teamSeasons || teamSeasons.length === 0) {
          return {
            status: 404,
            message: `${team.teamname.name} har inga säsonger i databasen.`,
            breadCrumb,
            meta: {
              description: `${team.teamname.name} har inga säsonger i databasen.`,
              title,
              url,
            },
          }
        ...
        breadCrumb = 'Säsonger'
        title = `Bandyresultat - ${team.teamname.name} - Säsonger`
        description = `Säsongslista för ${team.teamname.name}`
