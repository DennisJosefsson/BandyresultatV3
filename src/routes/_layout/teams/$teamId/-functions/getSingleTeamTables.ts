...
import { getUnionedTables } from './getTables'
  ...
  .handler(
    async ({ data: teamId }): Promise<TablesResponse> => {
      try {
        ...
        // const serieIdArray = await getTeamSerieIds(teamId)

        const unionedTables = await getUnionedTables({
          teamId,
        })

        if (unionedTables.length === 0) {
          return {
            status: 404,
        ...
        return { status: 200, tables: unionedTables }
      } catch (error) {
        catchError(error)
      }
    },
