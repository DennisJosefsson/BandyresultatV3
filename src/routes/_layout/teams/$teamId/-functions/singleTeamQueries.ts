...
export const getStatsCounts = async ({ team }: GetStringsProps) => {
  const TEAMID = team.teamId

  const getFirstDivisionSeasons = await getPreparedFirstDivisionSeasons.execute({
    teamId: TEAMID,
  })

  const qualificationSeasonsCount = await preparedIntersectSeasons
    .execute({ teamId: TEAMID })
    .then((result) => result.length)

  const firstDivSeasonsCount = parseFirstDivSeasons.parse(getFirstDivisionSeasons)

  const getFirstFirstDivisionSeason = await getPreparedFirstFirstDivisionSeason.execute({
    teamId: TEAMID,
  })

  const firstFirstDivisionSeason = parseFirstLatestFirstDivSeason.parse(getFirstFirstDivisionSeason)

  const getLatestFirstDivisionSeason = await getPreparedLatestFirstDivisionSeason.execute({
    teamId: TEAMID,
  })
  ...
  const finalCount = await preparedFinalCount.execute({ teamId: TEAMID }).then((result) => {
  ...
  const finalWinCount = await preparedFinalWinCount.execute({ teamId: TEAMID }).then((result) => {
    if (result.length === 0) return 0
    else return result[0].count
  })
  ...
  const playoffCount = await preparedPlayoffCount.execute({ teamId: TEAMID }).then((result) => {
    if (result.length === 0) return 0
    else return result[0].count
  })
