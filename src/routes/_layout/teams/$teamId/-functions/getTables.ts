...
import type { SingleTeamTableItem, singleTeamTable } from '@/lib/types/table'
...
export const getTables = async ({
  ...
  const filteredSeasons = seasonIdArray.filter((season) => !gameSeasonIds.includes(season))
  ...
  const teamTables = await db
    ...
    .where(and(eq(tables.teamId, teamId), inArray(tables.seasonId, filteredSeasons)))
        ...
        tables: sortCats[cat].reduce(
          (acc, curr) => {
            return {
              ...
              totalGoalsConceded: acc.totalGoalsConceded + curr.totalGoalsConceded,
              totalGoalDifference: acc.totalGoalDifference + curr.totalGoalDifference,
