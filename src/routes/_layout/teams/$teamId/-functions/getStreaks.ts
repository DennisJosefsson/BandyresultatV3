  ...
  const lost_values = db.$with('lost_values').as(
    db
      .select({
        ...
        lostValue: sql<number>`case when lost = true then 1 else 0 end`.as('lost_value'),
      })
      .from(teamgames)
  ...
  const summed_lost_values = db.$with('summed_lost_values').as(
    db
      ...
      .select({
        ...
        sumLosts: sql<number>`sum(lost_values.lost_value) over(partition by team order by date)`.as(
          'sum_losts',
        ),
  ...
  const group_array = db.$with('group_array').as(
    db
      ...
      .select({
        ...
        maxCount: sql<number>`mode() within group (order by grouped_losts.grouped)`.as('max_count'),
        dates: sql<Array<string>>`array_agg(date order by date)`.as('dates'),
      })
      .from(grouped_losts)
      .groupBy(grouped_losts.grouped, grouped_losts.teamId, grouped_losts.women),
  )
  ...
  const draw_values = db.$with('draw_values').as(
    db
      .select({
        ...
        date: teamgames.date,
        women: teamgames.women,
        drawValue: sql<number>`case when draw = true then 1 else 0 end`.as('draw_value'),
      })
      .from(teamgames)
      .where(and(eq(teamgames.played, true), eq(teamgames.teamId, teamId))),
  )
  ...
  const summed_draw_values = db.$with('summed_draw_values').as(
    db
      ...
      .select({
        ...
        sumdraws: sql<number>`sum(draw_values.draw_value) over(partition by team order by date)`.as(
          'sum_draws',
        ),
        round: sql<number>`row_number() over (partition by team order by date)`.as('round'),
      })
  ...
  const group_array = db.$with('group_array').as(
    db
      ...
      .select({
        ...
        maxCount: sql<number>`mode() within group (order by grouped_draws.grouped)`.as('max_count'),
        dates: sql<Array<string>>`array_agg(date order by date)`.as('dates'),
      })
      .from(grouped_draws)
      .groupBy(grouped_draws.grouped, grouped_draws.teamId, grouped_draws.women),
  )
  ...
  const win_values = db.$with('win_values').as(
    db
      .select({
        ...
        date: teamgames.date,
        women: teamgames.women,
        winValue: sql<number>`case when win = true then 1 else 0 end`.as('win_value'),
      })
      .from(teamgames)
      .where(and(eq(teamgames.played, true), eq(teamgames.teamId, teamId))),
  )
  ...
  const lost_values = db.$with('lost_values').as(
    db
      .select({
        ...
        date: teamgames.date,
        women: teamgames.women,
        lostValue: sql<number>`case when lost = false then 1 else 0 end`.as('lost_value'),
      })
      .from(teamgames)
      .where(and(eq(teamgames.played, true), eq(teamgames.teamId, teamId))),
  )
...
const getPlayoffStreak = async ({ teamId }: { teamId: number }) => {
  ...
  const selected_rows = db.$with('selected_rows').as(
    db
      ...
      .from(season_order)
      .leftJoin(playoff_seasons, eq(playoff_seasons.seasonId, season_order.seasonId)),
  )
