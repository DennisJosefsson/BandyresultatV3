...
const DataTable = <TData, TValue>({ columns, data }: DataTableProps<TData, TValue>) => {
  ...
  return (
    <div className="mb-1 ml-1 border px-1 shadow-xs sm:px-2 md:mb-2 md:ml-2 md:shadow-sm">
      <Table>
        <TableBody>
          {table.getRowModel().rows?.length ? (
            table.getRowModel().rows.map((row) => {
              ...
              const expandedInfo = !!original.extraTime || !!original.penalties

              return (
                <Fragment key={`${row.id}-${original.gameId}`}>
                  <TableRow
                    data-state={row.getIsSelected() && 'selected'}
                    data-favteam={
                      favTeams.includes(original.homeTeamId) ||
                      favTeams.includes(original.awayTeamId)
                        ? true
                        : false
                    }
                    data-expandedinfo={expandedInfo ? true : false}
                      ...
                      <Button
                        ...
                        render={
                          <route.Link
                            ...
                            search={(prev) => ({
                              ...prev,
                              teamArray: [original.homeTeamId, original.awayTeamId],
                            })}
                            state={{ origin: origin }}
                          >
                            <span>H2H</span>
                  ...
                  {row.getIsExpanded() && expandedInfo && (
                    <TableRow key={`${row.id}-expandedRow`}>
                      ...
                      >
                        Matchen slutade {original.result} efter full tid och avgjordes{' '}
                        {original.penalties
                          ? 'på straffar'
                          : original.extraTime
                            ? 'i förlängningen'
